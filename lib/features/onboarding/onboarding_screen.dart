...
class _OnboardingScreenState extends ConsumerState<OnboardingScreen> {
  ...
  void _nextPage() {
    ...
    if (currentPage == 9) {
      ...
      ref.read(onboardingProvider.notifier).setShowPermissionOverlay(true);
      ...
      body: SafeArea(
        child: Stack(
          children: [
            Column(
              children: [
                if (progressStep > 0 && state.currentPage != 5)
                  _buildProgressBar(progressStep),
                Expanded(
                  child: PageView(
                    ...
                    children: [
                      ...
                      OnboardingWallpaperPreview(onNext: _goToNext),
            ...
            if (state.showPermissionOverlay) _buildPermissionOverlay(),
          ],
        ),
      ),
    );
  ...
  Widget _buildPermissionOverlay() {
    return Positioned.fill(
      child: PermissionOverlay(
        onAllow: () {
          ref.read(onboardingProvider.notifier).setShowPermissionOverlay(false);
          _goToNext();
        },
        onDeny: () {
          ref.read(onboardingProvider.notifier).setShowPermissionOverlay(false);
